...
#define BOEFFLA_LEVEL_MIN			0
#define BOEFFLA_LEVEL_MAX			20
#define BOEFFLA_LEVEL_SHIFT			11
...
static ssize_t qpnp_boeffla_vib_level_show(struct device *dev,
                                        struct device_attribute *attr,
                                        char *buf)
{
        struct timed_output_dev *tdev = dev_get_drvdata(dev);
        ...
        return scnprintf(buf, PAGE_SIZE, "%d\n", vib->vtg_level - BOEFFLA_LEVEL_SHIFT);
}


static ssize_t qpnp_boeffla_vib_level_store(struct device *dev,
                                        struct device_attribute *attr,
                                        const char *buf, size_t count)
{
        struct timed_output_dev *tdev = dev_get_drvdata(dev);
        ...
        if (val < BOEFFLA_LEVEL_MIN) {
                pr_err("%s: level %d not in range (%d - %d), using min.", __func__, val, vib->vtg_min, vib->vtg_max);
                val = BOEFFLA_LEVEL_MIN;
        } else if (val > BOEFFLA_LEVEL_MAX) {
                pr_err("%s: level %d not in range (%d - %d), using max.", __func__, val, vib->vtg_min, vib->vtg_max);
                val = BOEFFLA_LEVEL_MAX;
        }

        vib->vtg_level = val + BOEFFLA_LEVEL_SHIFT;

        return strnlen(buf, count);
}

static DEVICE_ATTR(level, S_IRUGO | S_IWUSR, qpnp_boeffla_vib_level_show, qpnp_boeffla_vib_level_store);
...
static int qpnp_vib_set(struct qpnp_vib *vib, int on)
{
	...
	if ((on) && ((vib->vtg_level - BOEFFLA_LEVEL_SHIFT) > 0)) {
		val = vib->reg_vtg_ctl;
...
static int __devinit qpnp_vibrator_probe(struct spmi_device *spmi)
{
	...
	if (rc < 0)
		...
		device_create_file(vib->timed_dev.dev, &dev_attr_level);
